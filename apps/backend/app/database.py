...
from app.models import ApiKey, Application, GenerationTask, Improvement, Job, Resume
...
class Database:
    ...
    def _improvement_to_dict(row: Improvement) -> dict[str, Any]:
        return {
            ...
            "quality_audit": row.quality_audit,
            "created_at": row.created_at,
        }
    ...
    async def list_discovery_jobs(self, resume_id: str | None = None) -> list[dict[str, Any]]:
        """Return jobs created by the discovery workflow, newest first."""
        async with self._session() as session:
            statement = select(Job).order_by(Job.created_at.desc())
            if resume_id:
                statement = statement.where(Job.resume_id == resume_id)
            rows = (await session.execute(statement)).scalars().all()
            return [self._job_to_dict(row) for row in rows if (row.metadata_json or {}).get("discovery")]

    async def find_discovery_job_by_url(
        self, original_url: str, resume_id: str | None = None
    ) -> dict[str, Any] | None:
        """Find an imported job by canonical URL within its base-resume scope."""
        async with self._session() as session:
            ...
            for row in rows:
                ...
                if metadata.get("discovery") and metadata.get("original_url") == original_url:
                    return self._job_to_dict(row)
        return None

    async def find_discovery_job_by_identity(
        self, *, resume_id: str | None, title: str, company: str, location: str
    ) -> dict[str, Any] | None:
        """Fallback dedupe for sources which cannot provide a canonical URL."""
        for job in await self.list_discovery_jobs(resume_id):
            if (
                job.get("title") == title
                and job.get("company") == company
                and job.get("location") == location
            ):
                return job
        return None

    async def create_generation_task(self, resume_id: str, job_ids: list[str]) -> dict[str, Any]:
        task_id, now = str(uuid4()), _now()
        async with self._session() as session:
            session.add(GenerationTask(task_id=task_id, resume_id=resume_id, job_ids=job_ids, created_at=now, updated_at=now))
            await session.commit()
        return {"task_id": task_id, "resume_id": resume_id, "job_ids": job_ids, "status": "queued", "completed_job_ids": [], "generated_resume_ids": {}, "failed_jobs": {}, "current_stage": "queued", "current_job_id": None, "stage_detail": "等待开始", "created_at": now, "updated_at": now}

    @staticmethod
    def _generation_task_to_dict(row: GenerationTask) -> dict[str, Any]:
        return {
            "task_id": row.task_id, "resume_id": row.resume_id, "job_ids": row.job_ids,
            "status": row.status, "completed_job_ids": row.completed_job_ids,
            "generated_resume_ids": row.generated_resume_ids, "failed_jobs": row.failed_jobs,
            "current_stage": row.current_stage, "current_job_id": row.current_job_id,
            "stage_detail": row.stage_detail, "created_at": row.created_at, "updated_at": row.updated_at,
        }

    async def update_generation_task(self, task_id: str, **updates: Any) -> dict[str, Any] | None:
        async with self._session() as session:
            ...
            row.updated_at = _now()
            await session.commit()
            return self._generation_task_to_dict(row)
    ...
    async def list_recoverable_generation_tasks(self) -> list[dict[str, Any]]:
        """Tasks safely resumable after a local server restart."""
        async with self._session() as session:
            result = await session.execute(
            ...
            return [
                self._generation_task_to_dict(row)
                for row in rows
            ]

    async def list_generation_tasks(self, resume_id: str) -> list[dict[str, Any]]:
        """Recent batch tasks for one base resume, newest first."""
        async with self._session() as session:
            result = await session.execute(
                select(GenerationTask)
                .where(GenerationTask.resume_id == resume_id)
